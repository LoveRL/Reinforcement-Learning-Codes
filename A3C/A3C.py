...
T_max = 200         # Possible number of Max episodes
discount_factor = 0.9
...
'''
# A2C inheritents 'torch.nn.Module'
class A2C(nn.Module) :
...
class Each_local_worker(mp.Process) :
    ...
    def __init__(self, Global_A2C, Optimizer, name) :
    ...
    def run(self) :
        ...
        total_step = 1 # To check update step term
        
        for Local_epi_cnt in tq.tqdm(range(T_max)) :

            #print(self.name, Local_epi_cnt)
            ...
            for step in range(1, max_epi_step+1) :
                ...
                # because 'torch.nn' only provides mini-batch type
                ...
                reward_buffer.append((reward+10)/10) # reward normalization to reduce variance
                ...
                if total_step % t_max == 0 or done :
                    ...
                    # When an episode ends, update the global variables
                    if done :
                        break
...
if __name__ == '__main__' :
    ...
    Workers = [Each_local_worker(Global_A2C, Optimizer, i) for i in range(mp.cpu_count())]
